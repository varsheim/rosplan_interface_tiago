#include "RPCheckLight.h"

namespace KCL_rosplan {

    /* constructor */
    RPCheckLight::RPCheckLight(ros::NodeHandle &nh) {
        node_name = ros::this_node::getName();
        node_name_pretty = '(' + node_name + ')';
    }

    /* action dispatch callback */
    bool RPCheckLight::concreteCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
        ...
        // complete the action
        ROS_INFO("%s: CHECKLIGHT Action completing", node_name_pretty.c_str());
        return true;
    }
} // close namespace
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "rosplan_check_light_action_client", ros::init_options::AnonymousName);
    ros::NodeHandle nh("~");

    // create PDDL action subscriber
    KCL_rosplan::RPCheckLight rpti(nh);
